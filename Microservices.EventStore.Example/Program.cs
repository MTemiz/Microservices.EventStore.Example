...
// //
// //     await client.AppendToStreamAsync(streamName: "order-stream",
// //         expectedState: StreamState.Any,
// //         eventData: new[] { eventData });
// // }
//
// // var events = client.ReadStreamAsync(streamName: "order-stream",
// //     direction: Direction.Forwards,
// //     revision: StreamPosition.Start);
// //
// // var data = await events.ToListAsync();
//
// await client.SubscribeToStreamAsync(streamName: "order-stream",
...
//     },
//     subscriptionDropped: (streamSubscription, droppedReason, exception) => { Console.WriteLine("Disconnected"); }
// );
//     
...
// await eventStoreService.AppendToStreamAsync($"customer-{accountCreatedEvent.CustomerId}-stream",
//     new[]
//     {
...
await eventStoreService.SubscribeToStreamAsync($"customer-{accountCreatedEvent.CustomerId}-stream",
...
class EventStoreService
{
    EventStoreClientSettings GetEventStoreClientSettings(
        string connectionString = "esdb://admin:changeit@localhost:2113?tls=false&tlsVerifyCert=false") =>
        EventStoreClientSettings.Create(connectionString: connectionString);

    EventStoreClient Client
    {
        get => new EventStoreClient(GetEventStoreClientSettings());
    }
    ...
    public async Task SubscribeToStreamAsync(string streamName,
        ...
        await Client.SubscribeToStreamAsync(streamName: streamName,
            start: FromStream.Start,
            eventAppeared: eventAppeared,
            subscriptionDropped: (streamSubscription, droppedReason, exception) =>
            {
                Console.WriteLine("Disconnected");
            }
        );

    public EventData GenerateEventData(object @event) => new(eventId: Uuid.NewUuid(),
